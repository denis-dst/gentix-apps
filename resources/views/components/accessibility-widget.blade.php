<div x-data="{ 
    ...
    <div x-show="open" x-cloak @click.away="open = false" x-transition class="absolute bottom-full left-0 mb-4 bg-[#111118] border border-white/10 rounded-2xl p-4 shadow-2xl w-48 md:w-56 text-white">
        ...
        <button type="button" @click="toggleContrast()" aria-label="Toggle High Contrast" class="w-full text-left px-2 py-2 md:px-3 text-xs md:text-sm rounded-lg hover:bg-white/5 transition flex justify-between items-center mb-1">
        ...
        <button type="button" @click="toggleText()" aria-label="Toggle Large Text" class="w-full text-left px-2 py-2 md:px-3 text-xs md:text-sm rounded-lg hover:bg-white/5 transition flex justify-between items-center">
    ...
    <button type="button" @click="open = !open" aria-label="{{ app()->getLocale() == 'id' ? 'Buka Menu Aksesibilitas' : 'Open Accessibility Menu' }}" class="w-12 h-12 md:w-14 md:h-14 bg-gradient-to-br from-orange-500 to-amber-600 rounded-full flex items-center justify-center text-white shadow-lg shadow-orange-500/30 hover:scale-110 active:scale-95 transition-all">
