<x-app-layout>
    ...
    <div class="h-full w-full flex flex-col bg-[#010409] text-white overflow-hidden font-sans" x-data="gateScanner()">
        <!-- Header Bar -->
        <header class="h-14 sm:h-16 flex items-center justify-between px-4 sm:px-6 bg-[#0d1117] border-b border-white/5 shrink-0 z-50">
            <div class="flex items-center gap-3 sm:gap-4">
                <a href="{{ route('organizer.gate.setup', $event) }}" class="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center bg-white/5 hover:bg-white/10 rounded-xl transition text-slate-300">
                ...
                <div class="leading-tight">
                    ...
                    <p class="text-[8px] sm:text-[10px] font-bold text-orange-500 uppercase tracking-widest mt-0.5">{{ session('gate_name') }}</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 sm:gap-4">
                ...
                <div class="flex bg-black p-1 rounded-xl border border-white/5">
                    <button @click="setMode('IN')" :class="mode === 'IN' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-500 hover:text-slate-300'" class="px-3 sm:px-4 py-1 sm:py-1.5 rounded-lg text-[8px] sm:text-[10px] font-black transition-all uppercase tracking-widest">IN</button>
                    <button @click="setMode('OUT')" :class="mode === 'OUT' ? 'bg-orange-600 text-white shadow-lg shadow-orange-600/20' : 'text-slate-500 hover:text-slate-300'" class="px-3 sm:px-4 py-1 sm:py-1.5 rounded-lg text-[8px] sm:text-[10px] font-black transition-all uppercase tracking-widest">OUT</button>
                </div>

                <!-- Input Type Switcher -->
                <div class="flex bg-black p-1 rounded-xl border border-white/5">
                    <button @click="setInputType('auto')" :class="inputType === 'auto' ? 'bg-indigo-600 text-white' : 'text-slate-500'" class="p-1.5 sm:p-2 rounded-lg transition-all" title="Auto Scan">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" /></svg>
                    </button>
                    <button @click="setInputType('camera')" :class="inputType === 'camera' ? 'bg-indigo-600 text-white' : 'text-slate-500'" class="p-1.5 sm:p-2 rounded-lg transition-all" title="Camera Scan">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                    </button>
                    <button @click="setInputType('manual')" :class="inputType === 'manual' ? 'bg-indigo-600 text-white' : 'text-slate-500'" class="p-1.5 sm:p-2 rounded-lg transition-all" title="Manual Input">
            ...
            <div class="flex-1 flex flex-col min-h-0 relative bg-black overflow-hidden">
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center transition-all duration-300"
                    ...
                    <div class="absolute inset-0 flex items-center justify-center pointer-events-none z-30">
                        ...
                        <div x-show="status === 'idle' && inputType === 'auto'" class="flex flex-col items-center justify-center w-full h-full">
                        ...
                        <div x-show="status === 'idle' && inputType === 'camera'" class="flex flex-col items-center justify-center w-full h-full pointer-events-none">
                        ...
                        <!-- Success Overlay -->
                        <div x-show="status === 'success'" class="bg-[#065f46] fixed inset-0 flex flex-col items-center justify-center p-6 animate-in fade-in duration-200 z-50">
                        ...
                        <!-- Error Overlay -->
                        <div x-show="status === 'error'" class="bg-[#9f1239] fixed inset-0 flex flex-col items-center justify-center p-6 animate-in shake duration-300 z-50">
                        ...
                        <!-- Processing Overlay -->
                        <div x-show="status === 'processing'" class="bg-black/90 fixed inset-0 flex flex-col items-center justify-center backdrop-blur-xl z-50">
