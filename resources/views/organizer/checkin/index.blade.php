    ...
    <div class="space-y-6">
        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
            <form action="{{ route('organizer.checkin.index') }}" method="GET" class="flex gap-4">
                ...
                <button type="submit" class="px-8 py-3 bg-orange-600 text-black rounded-xl font-bold hover:bg-orange-700 transition shadow-lg shadow-orange-200">
                    Cari
                </button>
            </form>
        </div>
                                ...
                                <td class="px-6 py-4 text-right">
                                    @if($ticket->status === 'sold')
                                        <form action="{{ route('organizer.checkin.redeem', $ticket->id) }}" method="POST" onsubmit="return confirm('Konfirmasi penukaran tiket ini?')">
                                            @csrf
                                            <button type="submit" class="px-4 py-2 bg-orange-600 text-white text-xs font-bold rounded-lg hover:bg-orange-700 transition shadow-lg shadow-orange-100 flex items-center gap-2">
