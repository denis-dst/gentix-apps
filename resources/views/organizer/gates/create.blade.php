<x-app-layout>
    <x-slot name="title">Add Gate: {{ $event->name }}</x-slot>
    ...
    <div class="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-3xl font-black text-gray-800 tracking-tight">Add New Gate</h2>
                <p class="text-gray-500 font-medium">Create a new entry point and map ticket categories.</p>
            </div>
        ...
        <form action="{{ route('organizer.events.gates.store', $event) }}" method="POST" class="space-y-6">
            @csrf
            
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 space-y-6">
                <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Gate Name</label>
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Main Entrance, VIP Gate, North Gate" 
                ...
                <div>
                    ...
                    <textarea name="description" rows="2" class="w-full rounded-2xl border-gray-200 focus:border-orange-500 focus:ring-orange-500 transition-all py-3 px-4" placeholder="Briefly describe the gate location or purpose...">{{ old('description') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <p class="text-[10px] text-gray-400 mt-3 italic uppercase font-bold tracking-wider">Tickets from selected categories will be allowed to pass through this gate.</p>
                </div>
