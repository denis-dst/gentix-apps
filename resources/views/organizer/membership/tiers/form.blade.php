<x-app-layout>
    <x-slot name="title">{{ isset($tier) ? 'Edit Tier: ' . $tier->name : 'Tambah Tier Membership Baru' }}</x-slot>
    <x-slot name="header">{{ isset($tier) ? 'Edit Tier: ' . $tier->name : 'Tambah Tier Membership Baru' }}</x-slot>
    <x-slot name="actions">
        <a href="{{ route('organizer.membership.tiers.index') }}" class="min-h-[44px] inline-flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-bold transition focus:ring-2 focus:ring-slate-400">
