                                    ...
                                    <div class="p-5 bg-white rounded-3xl border border-slate-100 shadow-sm space-y-4">
                                        <div class="flex items-center justify-between border-b border-slate-50 pb-3">
                                            ...
                                            @if($tx->payment_status === 'paid' && $tx->tickets->where('status', '!=', 'void')->count() > 0)
                                                <form method="POST" action="{{ route('superadmin.transactions.cancel', $tx) }}" onsubmit="return confirm('KONFIRMASI SUPERADMIN: Anda akan membatalkan SELURUH transaksi ini dan mem-void semua tiket di dalamnya. Kuota akan otomatis dikembalikan ke stok. Lanjutkan?')">
                                                    @csrf
