                                ...
                                <td class="px-8 py-5">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if(!in_array($tx->payment_status, ['failed', 'expired', 'refunded']))
                                            <button type="button" 
                                                    onclick="openCancelModal('{{ $tx->id }}', '{{ $tx->reference_no }}', {{ json_encode($tx->tickets->where('status', '!=', 'void')->map(function($t) use ($tx) { return ['id' => $t->id, 'code' => $t->ticket_code, 'name' => $t->visitor_data['name'] ?? $tx->customer_name, 'category' => $t->category->name ?? '-']; })->values()) }}, '{{ route('superadmin.transactions.cancel-tickets', $tx) }}')"
