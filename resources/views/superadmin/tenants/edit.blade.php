<x-app-layout>
    <x-slot name="title">Edit Organizer: {{ $tenant->name }}</x-slot>

    <div class="bg-white rounded-sm border-t-4 border-orange-600 shadow-md overflow-hidden max-w-4xl mx-auto">
        <div class="px-6 py-4 border-b border-[#dee2e6] flex justify-between items-center bg-white">
            <h3 class="text-lg font-bold text-gray-800">Edit Organizer</h3>
        ...
        <div class="p-6">
            <form action="{{ route('superadmin.tenants.update', $tenant) }}" method="POST">
                ...
                <div class="mt-8 flex justify-end gap-3 border-t border-gray-100 pt-5">
                    <a href="{{ route('superadmin.tenants.index') }}" class="px-5 py-2.5 bg-orange-50 border border-orange-200 text-orange-700 font-bold rounded-lg hover:bg-orange-100 transition shadow-sm">Cancel</a>
