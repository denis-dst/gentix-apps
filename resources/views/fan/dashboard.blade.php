    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <a href="{{ route('fan.game-zone') }}" class="inline-flex items-center gap-2 min-h-[44px] px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-slate-950 font-black text-xs uppercase tracking-wider rounded-xl shadow-sm transition focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2">
                    Game Zone & Poin
                </a>
                <a href="{{ route('fan.season-pass') }}" class="inline-flex items-center gap-2 min-h-[44px] px-4 py-2.5 bg-orange-600 hover:bg-orange-700 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-sm transition focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2">
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            @if($member->kyc_status === 'unverified')
                <div class="bg-slate-900 text-white rounded-2xl p-6 border border-slate-800 shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div class="flex items-start sm:items-center gap-4">
                        <div class="w-12 h-12 bg-orange-600/20 border border-orange-500/30 rounded-xl flex items-center justify-center shrink-0">
                        ...
                        <div>
                            ...
                            <p class="text-xs sm:text-sm text-slate-300 mt-0.5">Sesuai regulasi Single Fan Identity PSSI, verifikasi identitas diperlukan untuk membeli dan mengklaim tiket pertandingan kandang.</p>
                        </div>
                    </div>
                    <a href="{{ route('fan.kyc') }}" class="min-h-[44px] inline-flex items-center justify-center px-5 py-2.5 bg-orange-600 hover:bg-orange-700 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow transition shrink-0 focus:outline-none focus:ring-2 focus:ring-orange-500">
                        Upload KYC (+100 Pts)
                    </a>
                </div>
            ...
            @if($pendingConsents->isNotEmpty())
                <div class="bg-slate-900 text-white rounded-2xl p-6 border border-slate-800 space-y-4">
                    ...
                    <div class="space-y-2">
                        @foreach($pendingConsents as $c)
                            <div class="bg-slate-950 rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-3 border border-slate-800">
                                <div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="space-y-6">
                    <div class="bg-slate-950 text-white rounded-2xl p-6 sm:p-7 shadow-lg border border-slate-800 relative">
                        <!-- Card Header -->
                        <div class="flex items-center justify-between mb-6">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-orange-600 rounded-xl flex items-center justify-center font-black text-base text-white">
                                    G
                                </div>
                                <div>
                                    <h4 class="font-outfit font-black tracking-wider text-sm">{{ $member->tenant->name ?? 'GENTIX CLUB' }}</h4>
                                    <p class="text-[10px] uppercase tracking-widest text-slate-400">Official Member Card</p>
                                </div>
                            </div>
                        ...
                        <div class="flex items-end justify-between border-t border-slate-800 pt-4">
                            ...
                            <div class="text-right">
                                ...
                                <p class="font-black text-base text-amber-400">{{ number_format($member->points_balance) }} Pts</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm space-y-4">
                        ...
                        <div class="p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
                            ...
                            <div class="text-right text-xs text-slate-500">
                                1 Poin = Rp 1<br>
                                <span class="text-emerald-600 font-bold">Dapat dipotong saat checkout</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-slate-900 text-white rounded-2xl p-6 border border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                        <div>
                            <span class="px-2.5 py-0.5 rounded-full bg-orange-600/20 text-orange-400 text-[10px] font-black uppercase tracking-wider">Mini Game Matchday</span>
