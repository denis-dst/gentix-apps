        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-black text-slate-900 font-outfit uppercase tracking-tight">
                    Tiket Terusan (Season Pass) & Klaim Matchday
                </h2>
                <p class="text-sm text-slate-500 font-medium">Akses tiket seluruh pertandingan kandang resmi klub selama 1 musim.</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            @forelse($seasonPasses as $sp)
                <div class="bg-white rounded-3xl p-8 border border-slate-200/80 shadow-sm space-y-6">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-6">
                        <div class="space-y-1">
                            <span class="px-3 py-1 bg-orange-100 text-orange-800 rounded-full text-xs font-black uppercase tracking-wider">
                            ...
                            <p class="text-xs font-mono text-slate-400">Kode Pass: <strong class="text-slate-800">{{ $sp->pass_code }}</strong> • Kategori: <strong class="text-slate-800">{{ $sp->category->name ?? 'Semua Tribun' }}</strong></p>
                        </div>
                        <div class="text-right">
                            <span class="text-xs text-slate-400 font-bold uppercase">Sisa Kuota Tiket</span>
                            <p class="text-3xl font-black text-slate-900 font-outfit">{{ $sp->total_matches - $sp->claimed_matches }} <span class="text-sm font-semibold text-slate-400">/ {{ $sp->total_matches }} Match</span></p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            @forelse($eligibleMatches as $match)
                                ...
                                <div class="bg-slate-50 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-3 border border-slate-200/60">
                                    <div>
                                        <div class="text-xs text-slate-400 font-semibold">{{ $match->event_start_date->format('l, d M Y • H:i') }} WIB • {{ $match->venue }}</div>
                                        <h5 class="text-sm font-bold text-slate-900 mt-0.5">{{ $match->name }}</h5>
                                    </div>

                                    <div class="flex items-center gap-2 shrink-0">
                                        @if($hasClaimed)
                                            <span class="px-4 py-2 bg-emerald-100 text-emerald-800 rounded-xl text-xs font-bold">
                                                ✓ Tiket Terklaim
                                            </span>
                                            @if($claimedRecord && $claimedRecord->ticket)
                                                <a href="{{ route('evoucher.public', $claimedRecord->ticket->transaction->reference_no ?? '') }}" target="_blank" class="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold transition">
                                                    Buka E-Voucher
                                                </a>
                                            @endif
                                        @elseif($isOpen)
                                            <form action="{{ route('fan.season-pass.claim', ['event' => $match, 'seasonPass' => $sp]) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="px-5 py-2.5 bg-orange-600 hover:bg-orange-700 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-lg shadow-orange-500/20 transition">
                                                    🎟 Klaim E-Ticket Sekarang
                                                </button>
                                            </form>
                                        @else
                                            <span class="px-4 py-2 bg-slate-200 text-slate-600 rounded-xl text-xs font-bold">
                            ...
                            @empty
                                <p class="text-xs text-slate-400 py-4">Belum ada pertandingan kandang yang dijadwalkan.</p>
                            @endforelse
                        </div>
                    </div>
                </div>
            @empty
                <div class="bg-white rounded-3xl p-12 text-center border border-slate-200">
                    ...
                    <h3 class="text-lg font-bold text-slate-800 font-outfit mb-1">Anda Belum Memiliki Season Pass</h3>
                    ...
                    <a href="{{ route('fan.dashboard') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-orange-600 hover:bg-orange-700 text-white rounded-xl text-sm font-bold shadow-lg shadow-orange-500/20 transition">
